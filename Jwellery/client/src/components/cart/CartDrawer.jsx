      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 bg-[#3E2024]/60 backdrop-blur-sm z-50"
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#EFE8DC]">
              <div className="flex items-center gap-3">
                <ShoppingBag size={20} className="text-[#C5A059]" strokeWidth={1.5} />
                <h2 className="font-light tracking-wide text-xl text-[#1F1517]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Your Cart</h2>
                {summary.totalItems > 0 && (
                  <span className="w-5 h-5 bg-[#C5A059] rounded-full text-white text-[10px] flex items-center justify-center font-bold">
                    {summary.totalItems}
                  </span>
                )}
              </div>
              <button onClick={() => setOpen(false)} className="p-1.5 hover:rotate-90 transition-all duration-300 text-[#1F1517] hover:text-[#C5A059]">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 bg-[#FAF8F3]">
              {!isAuthenticated ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                  <ShoppingBag size={48} strokeWidth={1} className="text-gray-300 mb-6" />
                  <p className="font-medium text-[#1F1517] mb-2 tracking-wide">Your cart is empty</p>
                  ...
                  {summary.amountForFreeShipping > 0 && (
                    <motion.div variants={itemVariant} className="bg-white border border-[#EFE8DC] rounded-sm p-4 mt-6">
                      <p className="text-[11px] text-[#6D6254] mb-3 tracking-wide">
                        Add <span className="font-semibold text-[#C5A059]">{formatPrice(summary.amountForFreeShipping)}</span> more for FREE shipping!
                      </p>
                    ...
                    {cart.coupon?.code ? (
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-white border border-[#EFE8DC] text-xs px-4 py-3 outline-none focus:border-[#C5A059] transition-colors rounded-sm tracking-wide text-[#1F1517]"
                        />
                        <button
                          onClick={handleApplyCoupon}
                          disabled={couponLoading}
                          className="bg-[#1F1517] text-white px-5 py-3 text-[10px] uppercase tracking-widest font-bold hover:bg-[#C5A059] transition-colors disabled:opacity-50 rounded-sm"
                        >
                          {couponLoading ? '...' : 'Apply'}
                        </button>
                      </div>
            ...
            {isAuthenticated && cart.items?.length > 0 && (
              <div className="border-t border-[#EFE8DC] px-6 py-6 bg-white shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex justify-between font-semibold text-[#1F1517] text-lg pt-4 border-t border-[#EFE8DC]">
                    <span>Total</span>
                    <motion.span 
                      key={summary.totalPrice} 
                      initial={{ scale: 1.1, color: '#C5A059' }} 
                      animate={{ scale: 1, color: '#1F1517' }} 
                      transition={{ duration: 0.3 }}
                    >
                ...
                <Link
                  to="/checkout"
                  onClick={() => setOpen(false)}
                  className="w-full bg-[#1F1517] text-white py-4 text-xs font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-[#C5A059] transition-colors rounded-sm shadow-md group relative overflow-hidden"
                >
                ...
                <Link
                  to="/cart"
                  onClick={() => setOpen(false)}
                  className="block text-center text-[10px] uppercase tracking-[0.2em] font-semibold text-[#6D6254] hover:text-[#C5A059] transition-colors mt-4"
                >
                  View full cart
                </Link>
              </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col py-1">
        <Link to={`/products/${product.slug}`}>
          <p className="text-xs font-medium text-[#1F1517] leading-snug line-clamp-2 hover:text-[#C5A059] transition-colors">{product.name}</p>
        </Link>
        ...
        <div className="flex items-end justify-between mt-3">
          ...
          <div className="flex items-center gap-1 border border-[#EFE8DC] rounded-sm bg-[#FAF8F3]">
            <button
              ...
              disabled={quantity <= 1}
              className="p-1.5 text-[#6D6254] hover:text-[#1F1517] disabled:opacity-30 transition-colors"
            >
            ...
            <motion.span 
              ...
              className="w-6 text-center text-xs font-medium text-[#1F1517]"
            >
              {quantity}
            </motion.span>
            <button
              onClick={() => onUpdate(quantity + 1)}
              className="p-1.5 text-[#6D6254] hover:text-[#1F1517] transition-colors"
            >
          ...
          {/* Price + Remove */}
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-[#1F1517]">{formatPrice(effectivePrice * quantity)}</span>
