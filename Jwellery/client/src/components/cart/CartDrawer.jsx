          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#EFE8DC]">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-light tracking-wide text-xl text-[#22181C]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Your Cart</h2>
              ...
              <button onClick={() => setOpen(false)} className="p-1.5 hover:rotate-90 transition-all duration-300 text-[#22181C] hover:text-[#C5A059]">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 bg-[#FAF8F3]">
              {!isAuthenticated ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                  <ShoppingBag size={48} strokeWidth={1} className="text-gray-300 mb-6" />
                  <p className="font-medium text-[#22181C] mb-2 tracking-wide">Your cart is empty</p>
                    ...
                    {cart.coupon?.code ? (
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-white border border-[#EFE8DC] text-xs px-4 py-3 outline-none focus:border-[#C5A059] transition-colors rounded-sm tracking-wide text-[#22181C]"
                        />
                        <button
                          onClick={handleApplyCoupon}
                          disabled={couponLoading}
                          className="bg-[#22181C] text-white px-5 py-3 text-[10px] uppercase tracking-widest font-bold hover:bg-[#C5A059] transition-colors disabled:opacity-50 rounded-sm"
                        >
                          {couponLoading ? '...' : 'Apply'}
                        </button>
                      </div>
            ...
            {isAuthenticated && cart.items?.length > 0 && (
              <div className="border-t border-[#EFE8DC] px-6 py-6 bg-white shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex justify-between font-semibold text-[#22181C] text-lg pt-4 border-t border-[#EFE8DC]">
                    <span>Total</span>
                    <motion.span 
                      key={summary.totalPrice} 
                      initial={{ scale: 1.1, color: '#C5A059' }} 
                      animate={{ scale: 1, color: '#22181C' }} 
                      transition={{ duration: 0.3 }}
                    >
                ...
                <Link
                  to="/checkout"
                  onClick={() => setOpen(false)}
                  className="w-full bg-[#22181C] text-white py-4 text-xs font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-[#C5A059] transition-colors rounded-sm shadow-md group relative overflow-hidden"
                >
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col py-1">
        <Link to={`/products/${product.slug}`}>
          <p className="text-xs font-medium text-[#22181C] leading-snug line-clamp-2 hover:text-[#C5A059] transition-colors">{product.name}</p>
        </Link>
        ...
        <div className="flex items-end justify-between mt-3">
          ...
          <div className="flex items-center gap-1 border border-[#EFE8DC] rounded-sm bg-[#FAF8F3]">
            <button
              ...
              disabled={quantity <= 1}
              className="p-1.5 text-[#6D6254] hover:text-[#22181C] disabled:opacity-30 transition-colors"
            >
            ...
            <motion.span 
              ...
              className="w-6 text-center text-xs font-medium text-[#22181C]"
            >
              {quantity}
            </motion.span>
            <button
              onClick={() => onUpdate(quantity + 1)}
              className="p-1.5 text-[#6D6254] hover:text-[#22181C] transition-colors"
            >
          ...
          {/* Price + Remove */}
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-[#22181C]">{formatPrice(effectivePrice * quantity)}</span>
