  ...
  return (
    <>
      ...
      <div className="space-y-6 max-w-4xl">
        <button onClick={() => navigate('/admin/orders')} className="flex items-center gap-2 text-sm text-[#756A63] hover:text-gray-800">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-gray-150 pb-5">
          <div>
            ...
            <p className="text-sm text-[#756A63] mt-1">Placed on {formatDate(order.createdAt)}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="space-y-3">
              {order.items?.map((item) => (
                <div key={item._id} className="flex gap-4 bg-white border border-[#DED3C4] rounded-2xl p-4 shadow-sm">
            ...
            {order.statusHistory?.length > 0 && (
              <div className="bg-white border border-[#DED3C4] rounded-2xl p-5 shadow-sm space-y-3">
                <h3 className="font-semibold text-gray-800 text-sm">Status Timeline</h3>
                <div className="relative border-l border-[#DED3C4] pl-4 ml-2 space-y-4 py-2">
                  {order.statusHistory.map((h, i) => (
                    <div key={i} className="relative">
                      ...
                      {h.note && <p className="text-xs text-[#756A63] mt-0.5">{h.note}</p>}
          ...
          <div className="space-y-4">
            {/* Pricing Details */}
            <div className="bg-white border border-[#DED3C4] rounded-2xl p-5 shadow-sm space-y-3">
              <h3 className="font-semibold text-gray-850 text-sm pb-2 border-b border-[#DED3C4]">Pricing</h3>
            ...
            {order.shippingAddress && (
              <div className="bg-white border border-[#DED3C4] rounded-2xl p-5 shadow-sm space-y-3">
                <h3 className="font-semibold text-gray-850 text-sm pb-2 border-b border-[#DED3C4]"><MapPin size={14} className="inline mr-1" /> Delivery Address</h3>
                ...
                <p className="text-xs text-[#756A63] flex items-center gap-1"><Phone size={11} /> {order.shippingAddress.phone}</p>
              </div>
            )}

            {/* Payment Details */}
            <div className="bg-white border border-[#DED3C4] rounded-2xl p-5 shadow-sm space-y-3">
              <h3 className="font-semibold text-gray-850 text-sm pb-2 border-b border-[#DED3C4]"><CreditCard size={14} className="inline mr-1" /> Payment</h3>
