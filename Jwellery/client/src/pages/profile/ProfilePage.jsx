      ...
      <div className="container-luxury py-10">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="lg:w-64 flex-shrink-0">
            <div className="bg-white rounded-2xl border border-[#DED3C4] p-5 text-center mb-4">
              ...
              <p className="text-xs text-[#756A63] truncate">{user?.email}</p>
              {user?.createdAt && <p className="text-xs text-gray-300 mt-1">Member since {formatDate(user.createdAt)}</p>}
            </div>
            <nav className="bg-white rounded-2xl border border-[#DED3C4] overflow-hidden">
              {tabs.map((tab) => {
                const Icon = tab.icon;
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 w-full px-5 py-3.5 text-sm transition-colors ${activeTab === tab.id ? 'bg-[#fdf9ee] text-[#C5A059] font-medium border-r-2 border-[#C5A059]' : 'text-[#756A63] hover:bg-gray-50'}`}
                  >
          ...
          <div className="flex-1 min-w-0">
            {activeTab === 'profile' && (
              <div className="bg-white rounded-2xl border border-[#DED3C4] p-6">
                ...
                {editing ? (
                  <div className="space-y-4 max-w-md">
                    <div><label className="text-xs text-[#756A63] block mb-1">Full Name</label><input value={profileData.name} onChange={(e) => setProfileData((p) => ({ ...p, name: e.target.value }))} className="input-gold" /></div>
                    <div><label className="text-xs text-[#756A63] block mb-1">Phone</label><input value={profileData.phone} onChange={(e) => setProfileData((p) => ({ ...p, phone: e.target.value }))} className="input-gold" /></div>
                  ...
                  <div className="space-y-4 text-sm">
                    {[{ label: 'Name', value: user?.name }, { label: 'Email', value: user?.email }, { label: 'Phone', value: user?.phone || 'Not provided' }, { label: 'Role', value: user?.role }].map(({ label, value }) => (
                      <div key={label} className="flex gap-4">
                        <span className="w-24 text-[#756A63]">{label}</span>
...
function NotificationsTab() {
  ...
  if (!notifications.length) return <div className="bg-white rounded-2xl border border-[#DED3C4] p-10 text-center text-[#756A63]"><Bell size={40} className="mx-auto mb-3 text-gray-200" />No notifications yet</div>;
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#DED3C4] p-6">
      ...
      <div className="space-y-3">
        {notifications.map((n) => (
          <div key={n._id} className={`p-3 rounded-xl border text-sm ${n.isRead ? 'border-[#DED3C4] bg-gray-50' : 'border-[#C5A059]/20 bg-[#fdf9ee]'}`}>
            <p className="font-medium text-gray-800">{n.title}</p>
            <p className="text-[#756A63] text-xs mt-0.5">{n.message}</p>
          </div>
        ))}
      </div>
    </div>
